...
export default function ProjectCard({ title, href, source, description, tags }: Props) {
    ...
    return (
        <div className="text-slate-900 space-y-4 h-full">
            <div className="space-y-2">
                ...
                <p>{description || 'lorem ipsum dolor sit amet consectetur adipisicing elit. Quisquam, quod.'}</p>
            </div>
            <div className="flex flex-wrap gap-x-2 text-sm text-slate-500">
                {tags.map((tag, index) => (
                    <span className="whitespace-nowrap" key={index}>{tag}</span>
                ))}
            </div>
            <div className="space-y-2 items-center mt-auto">
                {online !== false && (
                    <a href={href}
                        ...
                        {online !== true ? <AiOutlineLoading3Quarters className="w-6 h-6 animate-spin" /> : (
                            <>
